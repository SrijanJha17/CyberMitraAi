...
app.post('/analyse', async (req, res) => {
  try {
    let { text, _platform, field_type, _telegram_user } = req.body;
    if (!text || !text.trim()) return res.status(400).json({ error: 'No text provided' });

    const sourceLabel = _platform || 'Manual';
    const ftype = field_type || 'general'; // 'username' | 'dm' | 'post' | 'general'
    console.log(`\n📨 [${sourceLabel}/${ftype}] Analysis request: "${text.substring(0, 100)}..."`);

    const urlMatch = text.match(/(https?:\/\/[^\s]+)/i);
    // Cache key includes platform + field type so a username check never
    // collides with a DM check or post check on the same text
    ...
    if (cachedRow) {
      ...
      // Enrich with link content if URL present (only meaningful for 'post' type)
      if (urlMatch && ftype !== 'username') {
        const fetched = await fetchPostContent(urlMatch[0]);
        if (fetched) {
          text = `PLATFORM: ${urlMatch[0]}\n\nPOST CONTENT:\n${fetched}\n\nOriginal: ${text}`;
        }
      }

      try {
        analysis = await analyseWithGroq(text, ftype);
        console.log(`🤖 Groq → ${analysis.risk_level} | ${analysis.fraud_type}`);
      } catch (e) {
        console.error('❌ Groq API error:', e.message);
        // ✅ Fallback still broadcasts so dashboard always updates
        analysis = {
          risk_level: 'Medium',
          fraud_type: 'Analysis Unavailable',
          action_hindi: 'संदेश संदिग्ध लग रहा है। किसी विश्वसनीय व्यक्ति से सलाह लें।'
        };
      }

      // Save to DB (without _source, so cache stays clean/reusable)
      db.saveAnalysis({ cacheKey, source: sourceLabel, fieldType: ftype, text, analysis });
    }

    // Track Telegram users for analytics, if this request came from the bot
    if (_telegram_user && _telegram_user.id) {
      db.recordTelegramUser({
        chatId: String(_telegram_user.id),
        username: _telegram_user.username,
        firstName: _telegram_user.first_name,
        riskLevel: analysis.risk_level
      });
    }

    // Attach source label + field type for the live feed display, then broadcast + respond
